...
public static void register() {
	...
	children.put(Arrays.asList("delete"), CommandSpec.builder()
			.arguments(
				...
				public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
					...
					int i = vshop.getNPCfromLocation(loc);
					if (vshop.npcs.size() <= i) {
						src.sendMessage(Text.of(TextColors.RED, "Error! (There might be no shop at all)"));
					} else if (i < 0) {
						src.sendMessage(Text.of(TextColors.RED, "There is no shop at your location!"));
					} else {
						vshop.terminateNPCs();
						File f = new File("config/vshop/"+vshop.npcs.get(i).getIdentifier().toString()+".conf");
	...
	Sponge.getCommandManager().register(VillagerShops.getInstance(), CommandSpec.builder()
			...
			.extendedDescription(Text.of(
					...
					"ADD - <NIL|BuyPrice> [NIL|SellPrice] (Hold ItemStack, NIL will disable that option, one required)\n" +
