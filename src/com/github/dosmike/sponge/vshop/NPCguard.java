...
public class NPCguard {
	...
	public void setVariant(String fieldName) {
		if (fieldName.equalsIgnoreCase("none")) {
			...
			variant = StringUtils.isNumeric(fieldName) ? FieldResolver.getFinalStaticByIndex(HorseVariants.class, Integer.parseInt(fieldName)) : FieldResolver.getFinalStaticByName(HorseVariants.class, fieldName);
			...
			if (chunk.isPresent() && chunk.get().isLoaded()) {
				...
				if (le == null) { //unable to restore
				    ...
				    if (ent.isPresent()) {
				        ...
				        //setting variant. super consitent ;D
				        try {
				        if (npcType.equals(EntityTypes.HORSE)) {
							...
							((Villager)shop).career().set((Career)variant);
						}
