...
public class CommandRegistra {
    ...
    static void register() {
        ...
        Function<List<String>, Iterable<String>> CREATE_SkinSupplier = args->{
            for (Map.Entry<EntityType, Set<String>> entry : FieldResolver.autoTabCompleteMapping.entrySet()) {
                for (String arg : args) {
                            ...
                            player.sendMessage(Text.of(
                                    ...
                                    lang.localText("cmd.add.success")
                                            .replace("%item%", Text.of(TextColors.RESET, item.get().get(Keys.DISPLAY_NAME)
                                                    .orElse(Text.of(item.get().getType().getTranslation().get(VillagerShops.playerLocale(player)))), TextColors.GREEN))
                    ...
                    if (!npc.isPresent()) {
                        ...
                        src.sendMessage(Text.of(TextColors.GREEN, "[vShop] ",
                                lang.localText("cmd.identify.response")
                                        .replace("\\n", "\n")
                                        .replace("%type%", npc.get().getLe().getTranslation().get(VillagerShops.playerLocale(player)))
